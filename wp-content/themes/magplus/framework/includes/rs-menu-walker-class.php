...
class RS_Custom_Menu {
   ...
   */
    function save_custom_menu_items( $menu_id, $menu_item_db_id, $args ) {
      ...
      if (!isset($_REQUEST['edit-menu-item-icon_img_url'][$menu_item_db_id])) {
        $_REQUEST['edit-menu-item-icon_img_url'][$menu_item_db_id] = '';
      }
      $icon_img_url = $_REQUEST['edit-menu-item-icon_img_url'][$menu_item_db_id];
      update_post_meta( $menu_item_db_id, '_menu_item_icon_img_url', $icon_img_url );
  ...
  function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
    ...
    <li id="menu-item-<?php echo esc_attr($item_id); ?>" class="<?php echo magplus_sanitize_html_classes(implode(' ', $classes )); ?>">
      ...
      <div class="menu-item-settings" style="overflow:hidden;" id="menu-item-settings-<?php echo esc_attr($item_id); ?>">
        ...
        <!-- /Mega Menu item -->
        <?php
          $value = get_post_meta( $item->ID, '_menu_item_icon_img_url', true); 
          //if($depth == 0):
        ?>
        <div class="clearboth" style="clear:both;"></div>
        <div class="footer-menu-container">
          <p class="field-link-footer description description-wide">
            <label for="edit-menu-item-bg_img_url-<?php echo esc_attr($item_id); ?>">
              <?php esc_html_e( 'Icon Image URL', 'blogit' ); ?><br />
              <input class="widefat code" type="text" id="edit-menu-item-icon_img_url-<?php echo esc_attr($item_id); ?>" value="<?php echo esc_attr($value); ?>" name="edit-menu-item-icon_img_url[<?php echo esc_attr($item_id); ?>]" />
            </label>
          </p>
        </div>
        <?php //endif; ?>
...
class magplus_menu_widget_walker_nav_menu extends Walker_Nav_Menu {
  ...
  public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
    ...
    if ($item -> hide != 'enabled') {
      ...
      $item_output .= ($depth == 0 && !empty($item->icon_img_url)) ? '<img src="'.esc_url($item->icon_img_url).'">':'';
