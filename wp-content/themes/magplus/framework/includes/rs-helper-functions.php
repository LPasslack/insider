...
if(!function_exists('magplus_sidebar_position')) {
  function magplus_sidebar_position() {
    ...
    if(is_singular('post')) {
      ...
      $sidebar_details['layout']             = magplus_get_opt('search-sidebar-layout');
      $sidebar_details['sidebar-name-left']  = 'search-sidebar-left';
      $sidebar_details['sidebar-name-right'] = 'search-sidebar-right';
    } else {
...
if(!function_exists('magplus_getPostViews')) {
  function magplus_getPostViews($postID) {
    ...
    if($count == '' || $count == 0 ) {
      ...
      return wp_kses_post(magplus_get_opt('translation-zero-view'), 'magplus');
    }
    return ($count == 1 ) ? magplus_get_opt('translation-one-view'):$count.' '.magplus_get_opt('translation-views');
  }
}
...
/**
 * magplus_is_wocommerce
 */
if(!function_exists('magplus_is_wocommerce')) {
  function magplus_is_wocommerce() {
    if(!class_exists('WooCommerce')) { return; }
    return (is_product_category() || is_shop()) ? true:false;
  }
}
 ...
 * is_magplus_amp
 */
if(!function_exists('is_magplus_amp')) {
  function is_magplus_amp() {
    return (function_exists('is_amp')) ? is_amp():false;
  }
}

if(!function_exists('magplus_custom_font')) {
  function magplus_custom_font() {
    $font_face    = '';
    $custom_fonts = magplus_get_opt('custom_fonts');
    if(isset($custom_fonts['name']) && is_array($custom_fonts['name'])) {
      foreach ($custom_fonts['name'] as $key => $font_family) {
        $process = false;
        foreach (array('woff', 'woff2', 'ttf', 'svg', 'eot') as $file_type) {
          if(!$process && isset($custom_fonts[$file_type]) && isset($custom_fonts[$file_type][$key])) {
            $process = true;
          }
        }
        if(!$process) {
