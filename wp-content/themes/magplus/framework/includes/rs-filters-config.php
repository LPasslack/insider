 ...
 * @since 2.5
 */
if(!function_exists('magplus_custom_fonts')) {
  function magplus_custom_fonts($array) { 
    $custom_font_array = array();
    $custom_fonts      = magplus_get_opt('custom_fonts');
    if(!empty($custom_fonts['name']) && is_array($custom_fonts['name'])) {
      foreach($custom_fonts['name'] as $name) {
        $custom_font_array[$name] = $name;
      }
    }
    if(!empty($custom_font_array)) {
      $array = array('Custom Fonts' => $custom_font_array);
    }
    return $array; 
  }
  add_filter('redux/magplus_theme_options/field/typography/custom_fonts', 'magplus_custom_fonts', 10, 1);
}
